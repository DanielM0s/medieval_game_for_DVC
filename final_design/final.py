            ...
            while True:
                try:
                    ...
                    if angle > 90:
                        ...
                        self.health -= 10
                    height_of_arrow = [self.safe_calculate(force, length, self.arrow.weight, angle, dist) for dist in
                                    range(1, int(self.enemy.distance) + 1)]
...
class shop_main:
    ...
    bow = MedievalShopItem("longbow", 20, 710, 0.762)
            ...
            while enemy.check_health():
                while True:
                    ...
                    if attack in attack_types:
                        ...
                        elif force > enemy.defense:
                            print("you have not provided enough force to get through the enemies defense")
                        else:
                            try:
...
def medieval_shop():
    ...
    while True:
        ...
        if choice == "1":
            buy_item(player.coins, player.inventory, shop_main.shop)
        elif choice == "2":
            sell_item(player.coins, player.inventory, shop_main.shop)
        elif choice == "3":
            upgrade_inventory(player.coins, player.inventory, shop_main.shop)
        elif choice == "4":
            print("Goodbye!")
            break
        else:
    ...
    if choice < 0 or choice >= len(shop.items):
        ...
        if item.price <= coins:
            ...
            if answer.lower() == "y":
                ...
                print(f"You have {player.coins} coins left.")
        else:
...
def sell_item(coins, inventory, shop):
    ...
    if item_name in inventory:
        ...
        if price:
            ...
            print(f"You have {player.coins} coins now.")
        else:
...
enemy = bow_calculator.bow_enemy("enemy",2.5, random.randint(300, 400))
archer = bow_calculator.Archer(shop_main.bow, shop_main.arrow, enemy, 100)
...
while True:
    ...
    print("2. Shoot the enemy")
    print("3. Leave")

    choice = input("Enter your choice: ")

    if choice == "1":
        fight_main.main_fight(player, fighting.Enemy())
    elif choice == "2":
        archer.attack()
    elif choice == "3":
        break
